...
func Test_getJulianDay(t *testing.T) {
	...
	scenarios := map[time.Time]float64{
		...
		time.Date(1582, 10, 15, 0, 0, 0, 0, time.UTC):  2299170.5,
		...
		time.Date(2009, 6, 12, 12, 0, 0, 0, jkt):       2454994.708,
	}

	for date, expected := range scenarios {
		result := getJulianDay(date)

		result = round(result, 3)
		expected = round(expected, 3)

		if result-expected != 0 {
			t.Errorf("%s expected %f get %f",
